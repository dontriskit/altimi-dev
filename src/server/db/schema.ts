...
// Modernization intake form submissions
export const intakeSubmission = sqliteTable(
  "intake_submission",
  (d) => ({
    id: d.integer({ mode: "number" }).primaryKey({ autoIncrement: true }),
    ...
    role: d.text({ length: 255 }),
    currentStack: d.text(), // JSON array
    primaryDriver: d.text({ length: 255 }),
    previousAttempts: d.text({ length: 255 }),
    teamSize: d.text({ length: 255 }),
    timelinePressure: d.text({ length: 255 }),
    areasFocus: d.text(), // JSON array
    outputNeeded: d.text({ length: 255 }),
    createdAt: d
      .integer({ mode: "timestamp" })
      .default(sql`(unixepoch())`)
      .notNull(),
  }),
  (t) => [index("intake_email_idx").on(t.email)]
);
